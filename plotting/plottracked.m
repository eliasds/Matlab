...
xyzLoc = xyzLocSmoothed; % change to struct with your data
...
zmin = min(z1,z2);
figure(fignum);
for La = 1:numel(xyzLoc)
%     scatter3(xscale*xyzLoc(La).time(:,1),zscale*(-z2+xyzLoc(La).time(:,3)),yscale*(ymax-xyzLoc(La).time(:,2)),30,'filled','b'); %Blue Particles
    scatter3(xscale*xyzLoc(La).time(:,1),zscale*(-zmin+xyzLoc(La).time(:,3)),yscale*(ymax-xyzLoc(La).time(:,2)),30,'filled','CData',xyzLoc(La).time(:,4)); colormap(jet(125)); %Multicolored Particles
    view([az,el])
    axis equal
