function [o_slice, P, err] = AlterMin_MultiSlice( I, No, Nslice, Ns, H, H0, opts)
...
if opts.display
    if ~isfield(opts,'displayslice')
        m0 = Nslice;
    else
        m0 = opts.displayslice;
    end
end

if opts.display
    f1 = figure(88);
    subplot(231); imagesc(abs(o_slice(:,:,m0))); axis image; colormap gray; 
    h= colorbar; set(h,'fontsize',8); axis off;
%     title(['slice',num2str(m0)],'fontsize',10); axis off;
    if m0>1
        for m = 1:min(m0-1,3)
            subplot(2,3,m+1); imagesc(abs(o_slice(:,:,m0-m))); axis image; colormap gray; 
            h= colorbar; set(h,'fontsize',8); axis off;
%             title(['slice',num2str(m0-m)],'fontsize',10);axis off;
        end
    else
        subplot(232); imagesc(angle(o_slice(:,:,m0))); axis image; colormap gray; colorbar; 
        h= colorbar; set(h,'fontsize',8); axis off;
%         title('phase(o)','fontsize',10); axis off;
    end
    subplot(235); imagesc(abs(P)); axis image; colormap gray; colorbar; 
    h= colorbar; set(h,'fontsize',8); axis off;
%     title('ampl(P)'); axis off; 
    subplot(236); imagesc(angle(P).*abs(P)); axis image; colorbar; 
    h= colorbar; set(h,'fontsize',8); axis off;
...
while abs(err1-err2)>opts.tol&&iter<opts.maxIter
    ...
    for m = 1:Nimg
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [phi_fwd, psi_fwd] = Fwd_Prop_MultiSlice( i0, o_slice, H, H0);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        o_slice = Back_Prop_MultiSlice(O, H, H0, o_slice, phi_fwd, psi_fwd, ...
    ...
    if strcmp(opts.display,'full')
        f1 = figure(88);
        subplot(231); imagesc(abs(o_slice(:,:,m0))); axis image; colormap gray; colorbar; 
        h= colorbar; set(h,'fontsize',8); axis off;
%         title(['slice',num2str(m0)]); axis off;
        if m0>1
            for m = 1:min(m0-1,3)
                subplot(2,3,m+1); imagesc(abs(o_slice(:,:,m0-m))); axis image; colormap gray; colorbar; 
                h= colorbar; set(h,'fontsize',8); axis off;
%                 title(['slice',num2str(m0-m)]);axis off;
            end
        else
            subplot(232); imagesc(angle(o_slice(:,:,m0))); axis image; colormap gray; colorbar; 
            h= colorbar; set(h,'fontsize',8); axis off;
%             title('phase(o)'); axis off;
        end
        subplot(235); imagesc(abs(P)); axis image; colormap gray; colorbar; 
        h= colorbar; set(h,'fontsize',8); axis off;
%         title('ampl(P)'); axis off;
