function [O, P, err, scale] = AlterMinSeq_AdapScale_UpSamp( I, Ns, opts)
...
%
% up-sample the measurement to match the pixel res of the reconstruction
% before fed into the algorithm
...
%     opts.O0 = padarray(opts.O0,(No-No)/2);
    opts.P0 = ones(No);
    opts.alpha = 1;
    opts.beta = 1;
    opts.mode = 'real';
...
iters = opts.iters;
if opts.adaptive
    P1 = @(psi,I,c,mode) Proj_Fourier_AdapScale(psi, I, c, mode);
else
    P1 = @(psi,I) Proj_Fourier(psi, I);
end
P2 = @(O,P,psi,psi0,s)...
    GDUpdate_Multiplication_UpSamp(O,P,psi,psi0,s,Ps,alpha,beta,iters);
%P2 = @(O,P,psi,cen) MinUpdate_Multiplication(O,P,psi,cen,Ps,iters);
% % operator to put P at proper location at the O plane
% upsamp = @(x,cen) padarray(padarray(x,(No-No)/2-(cen0-cen),'pre')...
%     ,(No-No)/2+(cen0-cen),'post');
% operator to crop region of O from proper location at the O plane
% downsamp = @(x,cen) x(cen(1)-No(1)/2:cen(1)+No(1)/2-1,...
%     cen(2)-No(2)/2:cen(2)+No(2)/2-1);
...
% intensity parameter 
scale = opts.scale;
% led index for each measurement
litidx = opts.ledidx;
% init pupil and object 
P = opts.P0; opts.P0 = 0;
O = opts.O0; opts.O0 = 0;
err1 = inf;
err2 = 50;
...
while abs(err1-err2)>opts.tol&&iter<opts.maxIter
    ...
    for m = 1:Nimg
        ...
        psi0 = zeros(No(1),No(2),r);
%         cen = zeros(2,r);
        s = zeros(2,r);
        for p = 1:r
%             cen(:,p) = cen0+row(Ns(p,m,:));
            s(:,p) = row(Ns(p,m,:));
%             psi0(:,:,p) = O.*circshift(P,s(:,p));
            % construct psi0, note that based on the physical process, the
            % spectral of the object O is shifted according to the carrier
            % frequency, pupil P is always fixed
            psi0(:,:,p) = P.*circshift(O,-s(:,p));
        end
        % measured intensity
        I_mea = I(:,:,m);
...
%             c_vec(m,iter) = c;
        ...
        if strcmp(opts.display,'full')
            ...
            subplot(224); imagesc(angle(P).*abs(P)); axis image; colormap jet; colorbar;
            title('phase(P)');
            figure(12); imagesc(log(abs(O))); axis image; 
            drawnow;
        end
        %end
