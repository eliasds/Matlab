function [J,P] = deconvblind_adjust(varargin)
...
wI = (WEIGHT.*(READOUT + J{1}));% at this point  - positivity constraint
fw = fftn(WEIGHT);
clear WEIGHT;
DAMPAR22 = (DAMPAR.^2)/2;
...
for k = (lambda + 1) : (lambda + NUMIT),
  ...
  if k > 2,% image
    ...
    lambda = (min(lambda,1));% stability enforcement
  end
  Y = (J{2} + lambda*(J{2} - J{3}));% image positivity constraint
  
  if k > 2,% PSF
    lambda = (P{4}(:,1).'*P{4}(:,2))/(P{4}(:,2).'*P{4}(:,2) + eps);
    lambda = (min(lambda,1));% stability enforcement
  end
  B = (P{2} + lambda*(P{2} - P{3}));% PSF positivity constraint
  sumPSF = sum(B(:));
  ...
  J{3} = J{2};
  H = psf2otf(P{2},sizeI);
  scale = (ifftn(conj(H).*fw)) + sqrt(eps);
  J{2} = (Y.*(ifftn(conj(H).*CC))./scale);
  clear scale;
  J{4} = [J{2}(:)-Y(:) J{4}(:,1)];
  clear Y H;
  ...
  P{2} = (B.*otf2psf(conj(H).*CC,sizePSF)./scale);
  clear CC H;
...
iptchecknargin(2,inf,nargin,mfilename);% no constraint on max number 
                                       % because of FUN args
  ...
  if ~isempty(idx),
    ...
    try % how this function works, just in case.
      ...
      msg = sprintf(['DECONVBLIND cannot continue because user supplied' ...
               ...
               Ftype{idx},Ffcnstr{idx},ME.message); 
...
%
iptchecknargin(3,7,funnum,mfilename);
switch funnum,
