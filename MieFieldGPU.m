function E = MieFieldGPU(n1, n2, d, lambda, Hsize, dpix, Z, cen, digit)
...
%   Attempted implementation of built-in gpuArray programming
%
%%
if nargin == 7
    digit = 4;
...
ucompute = gather(cos(thetacompute));
num = gather(length(ucompute));

Nmax=gather(round(2+alpha+4*alpha.^(1/3)));
N=(1:Nmax);
N2NN=(2*N+1)./(N.*(N+1));
...
px=sx.*besselj(gather(nu),gather(alpha));
