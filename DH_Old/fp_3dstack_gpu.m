%% Fresnel Propagation into a 3D stack on CUDA/NVIDIA GPU Function.
% Version 1.0
...
function [fp3d] = fp_3dstack_gpu(E0,lambda,Z,ps,zpad,Zchunks)
...
fp3d=gpuArray(zeros(m,n,length(Z)));
...
for z = 1:length(Z)
    ...
    E1_gpu=real(E1temp(1+(M-m)/2:(M+m)/2,1+(N-n)/2:(N+n)/2)); %real, unpadded reconstruction intensity
