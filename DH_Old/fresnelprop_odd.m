...
% Create Frequency Space for even and odd total #pixel cases
if mod(N,2) && mod(M,2)        % N & M odd case
    [x,y]=meshgrid(-(N-1)/2:((N-1)/2), -(M-1)/2:((M-1)/2));
elseif mod(N,2) && mod(M,2)==0 % N odd,  M even case
    [x,y]=meshgrid(-(N-1)/2:((N-1)/2), -M/2:(M/2-1));
elseif mod(N,2)==0 && mod(M,2) % N even, M odd case
    [x,y]=meshgrid(-N/2:(N/2-1), -(M-1)/2:((M-1)/2));
else                           % N & M even case
