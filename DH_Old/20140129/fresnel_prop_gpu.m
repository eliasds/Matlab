function [Ef] = fresnel_prop_gpu(E0,lam,zall,ps)
...
%     H(:,:,z) =  exp(1i*(2*pi/lam)*z)* exp(-1i*pi*lam*zall(z)*(Fx.^2+Fy.^2));
%
% end
...
for z = 1:length(zall)
    g(:,:,z) = ifft2(ifftshift( (exp(1i*(2*pi/lam)*z)*exp(-1i*pi*lam*zall(z)*(Fx.^2+Fy.^2))) .* E0fft));
end
