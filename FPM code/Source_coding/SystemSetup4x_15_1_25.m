...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mag = 8.1458; % calibrated on 2/2/2015
...
idx_left = find(idx_u(LitCoord)>0);
idx_right = find(idx_u(LitCoord)<0);
idx_top = find(idx_v(LitCoord)>0);
idx_bottom = find(idx_v(LitCoord)<0);
...
for m = 1:32
    for n = 1:32
        if idx_v2(m,n)>0 && idx_u2(m,n)>0 && idx_v2(m,n)<Np && idx_u2(m,n)<Np
