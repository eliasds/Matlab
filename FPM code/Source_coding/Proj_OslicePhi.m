function [ O,phi ] = Proj_OslicePhi(O,phi,dpsi,alpha,beta)
...
% dpsi = psi-psi0;
O = O+abs(phi).*conj(phi)./(abs(phi).^2+alpha).*dpsi/max(abs(max(phi)));
phi = phi+abs(O).*conj(O)./(abs(O).^2+beta).*dpsi/max(abs(max(O)));
