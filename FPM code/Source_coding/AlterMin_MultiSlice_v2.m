function [o_slice, P, errall] = AlterMin_MultiSlice_v2( I, No, Ns, k2, dz, H0, lambda, opts)
...
Proj_FT = @(f,I) F(sqrt(I).*f./(abs(f)+eps));
...
%     err2 = 0;
    ...
    for m = 1:Nimg
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [phi_fwd, psi_fwd] = Fwd_Prop_MultiSlice_v2( i0, o_slice, k2, dz, lambda, opts.Prop_mode);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         err2 = err2+sum((I_mea(:)-I_est(:)).^2);
        ...
        dG = G_new-G_fwd;
        % location of DC
        c0 = squeeze(Ns(:,m,:))'+cen0;
        Omax = abs(O(c0(1),c0(2)));
        [O,P] = Proj_ObjPupil(O,P,dG,Ps,Omax,opts.OP_alpha,opts.OP_beta);
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % step 3: back-propgate field based on multi-slice approach & update
        % estimationo of o_slice
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        o_slice = Back_Prop_MultiSlice_v2(O, k2, dz, lambda, o_slice, phi_fwd, psi_fwd, ...
            i0, opts.BP_alpha, opts.BP_beta, opts.Prop_mode);
...
for m = 1:Nimg
    ...
    %           compute incident (phi) and output (psi) field for each slice
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    I_est = Fwd_Prop_MultiSlice_Intensity(i0, o_slice, k2, dz, lambda, P, H0, opts.Prop_mode);
