function [o_slice, P, err, shift_vec] = AlterMin_MultiSlice_v3( I, No, Ns, k2, dz, H0, opts)
...
%         Nshift = [Ns(:,m,1), Ns(:,m,2)];
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % LED position/illumination angle/spatial freq of illumination
        % calibration with sub-pixel accuracy
        % ref: F.Zhang,et.al, OE 2013
        % step b) implement the calibration/correction
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if opts.PosCalib
%             O1 = downsamp(O).*Ps;
            [output, ~] = dftregistration(fft2(I_est),fft2(I_mea),opts.PosRes);
%             Ns(:,m,2) = Ns(:,m,2)+output(4);
%             Ns(:,m,1) = Ns(:,m,1)+output(3);
            % record the result
            shift_vec(1,m,iter+1) = output(3);
            shift_vec(2,m,iter+1) = output(4);
        end
...
%         err2 = err2+sum((I_mea(:)-I_est(:)).^2);
        ...
        % calibration with sub-pixel accuracy
        % ref: F.Zhang,et.al, OE 2013
        % step a) save previous O before pass to obj_pupil update
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         if opts.PosCalib
%             O0 = downsamp(O).*Ps;
%         end
...
%         if opts.PosCalib
%             O1 = downsamp(O).*Ps;
%             [output, ~] = dftregistration(fft2(O0),fft2(O1),opts.PosRes);
%             Ns(:,m,2) = Ns(:,m,2)+output(4);
%             Ns(:,m,1) = Ns(:,m,1)+output(3);
%             % record the result
%             shift_vec(1,m,iter+1) = output(3);
